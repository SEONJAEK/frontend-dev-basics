...
$(function(){
	//DOM Load Event
	//1. load: 모두다(DOM, CSSOM, Img) 로딩
	//2. DOMContentLoaded: DOM만 로딩(CSSOM x, image x)
	window.addEventListener("DOMContentLoaded", function(){
	
		document
		.getElemenstByTageName("button")[0]
		.addEventListener("click", function(){
			
			var xhr = null;
			if(window.ActiveXObject){ // <=IE9
				xhr = new ActiveXOjbect("Microsft.XMLHTTP");
			...
			xhr.addEventListener('readystatechange', function(){
				...
				var response = JSON.parse(this.responseText); //객체로 변화시키는 컨버터
				//console.log(response, typeof(responseText));
...
<body>
	<h1>AJAX Test: JSON Format Data: XmlHttpRequest직접 사용하기</h1>
